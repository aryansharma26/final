    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 px-4 py-2 text-sm font-semibold text-white bg-brand rounded-xl hover:bg-brand-dark focus:outline-none transition-all whitespace-nowrap disabled:opacity-50 cursor-pointer h-[38px] sm:h-auto"
        >
          {exportLoading ? (
                  ...
                  <tr key={item._id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleView(item)} className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-brand/10 hover:text-brand" title="View">
                          {viewingId === item._id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Eye className="h-4 w-4" />}
                        </button>
                        <button onClick={() => handleDownload(item)} className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700" title="Download">
                          {downloadingId === item._id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}
                        </button>
                        <button onClick={() => openReview(item)} className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-blue-50 hover:text-blue-600" title="Review"><Edit2 className="h-4 w-4" /></button>
                        <button onClick={() => openQuote(item)} className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-amber-50 hover:text-amber-600" title="Send Product Quote">
                          {quoteLoading && quoteItem?._id === item._id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                        </button>
                        <button onClick={() => setDeleteId(item._id)} className="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600" title="Delete"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      <Modal open={!!quoteItem} onClose={() => setQuoteItem(null)} title="Send Medicine Quote" maxWidth="max-w-3xl">
        {quoteItem && (
          <div className="space-y-4">
            ...
            <button type="button" onClick={addQuoteRow} className="inline-flex items-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 hover:border-brand/30 hover:text-brand">
