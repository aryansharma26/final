...
const CategoriesModule = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      showMessage(err.response?.data?.message || 'Failed to save category');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showMessage(err.response?.data?.message || 'Failed to delete category');
    } finally {
  ...
  const getDeleteMessage = () => {
    ...
    if (!category) return 'Are you sure you want to delete this category?';
    const childCount = categoriesByParent[String(category._id)]?.length || 0;
    if (childCount > 0) {
      return `Delete "${category.name}" and its ${childCount} subcategories? Products inside them will be marked inactive.`;
    }
    return `Delete "${category.name}"? Products inside it will be marked inactive.`;
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage parent categories and subcategories shown across the shop.</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5}><EmptyState icon={FolderTree} title="No categories found" subtitle="Create your first category to start organizing products." /></td></tr>
              ) : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingCategory ? 'Edit Category' : 'Add Category'} maxWidth="max-w-xl">
